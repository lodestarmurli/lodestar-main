...
public class ChanakyaEdupathDAO {
	private static final Logger	OUT	= LoggerFactory.getLogger(ChanakyaEdupathDAO.class);

	public static List<DHEdupathDTO> getDHEdupathbyOccId(List<Integer> occupationIds ) throws Exception
	{
		List<DHEdupathDTO> list = MyBatisManager.getInstance().getResultList(MyBatisMappingConstants.GET_DHEDUPATH, occupationIds);
		return list;
	}
	
	public static List<DHEntExamDTO> getexamdescription(List<String> examList ) throws Exception
	{
		List<DHEntExamDTO> list = MyBatisManager.getInstance().getResultList(MyBatisMappingConstants.GET_DHEXAM_LIST, examList);
		return list;
	}

...
public class ArchiveStudentDetailsDAO
{
	private static final Logger	OUT	= LoggerFactory.getLogger(ArchiveStudentDetailsDAO.class);
	...
	public List<StudentDetailsDTO> getStudentListForSummary(Map<String, Object> paramMap, int skipStrat, int maxLimit) throws Exception
	{
		...
		List<StudentDetailsDTO> list = MyBatisManager.getInstance().getResultList(MyBatisMappingConstants.STUDENT_GET_SUMMARY_Archive, paramMap, skipStrat, maxLimit);
	...
	public StudentDetailsDTO getStudentDetailsByStudentId(int id) throws Exception
	{
	...
	{
		OUT.debug("Get Student Details For Summary With serach Param : {}", paramMap);
		Integer count = MyBatisManager.getInstance().getUniqueCount(MyBatisMappingConstants.STUDENT_GET_SUMMARY_COUNT_Archive, paramMap);

...
public class TrialStudentSubmitAction extends BaseAction implements ModelDriven<StudentDetailsDTO>
{
	private static final long			serialVersionUID	= 4228877756086138530L;
	private static Logger				OUT					= LoggerFactory.getLogger(TrialStudentSubmitAction.class);
	...
	public TrialStudentSubmitAction()
	{
		...
		try
		{
			...
			if (studentDTO.getSession3DateStr() != null) {
				session3Count = new SessionScheduleDetailsDAO().getcountBySessionByDate(studentDTO.getId(),
						studentDTO.getFacilitatorId(),TimeUtil.getDate(studentDTO.getSession3DateStr(), TimeUtil.FILTER_DATE_TIME_FORMAT), 3);
			}

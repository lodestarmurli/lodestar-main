...
public class StudentExcelValidator
{
	private static Logger		OUT				= LoggerFactory.getLogger(StudentExcelValidator.class);
	...
	{
		StudentExcelUploadDBUtil util = new StudentExcelUploadDBUtil();
...
//				}
				...
				if (null != typeEnum && StudentTypeEnum.FULL == typeEnum)
				{
					...
					if (!isSession3DateValid(rowDataMap, xlsEngine, propInstance, propInstance.getProperty("com.edupath.bulkupload.column.session3date"),className))
					{
					...
					if (!isSession3TimeValid(rowDataMap, xlsEngine, propInstance, timeList, propInstance.getProperty("com.edupath.bulkupload.column.session3time"),className))
					{
	...
	private boolean isDateTimeValidGap(Map<String, NetValenceExcelCellVO> rowDataMap, XLSEngine xlsEngine, ApplicationProperties propInstance, int minDays)
	{
		...
		try
		{
			...
			try
			{
				...
				{
					date3 = new Date(TimeUtil.getDate(session3Date, TimeUtil.EXCEL_DATE_FORMAT).getTime());
					time3 = new Date(TimeUtil.getDate(session3time, TimeUtil.EXCEL_DATE_FORMAT).getTime());
				}
			...
			List<FacilitatorDetailsDTO> facilitatorListDTO, String sessionName)
	{
		...
		if (sessionName.equals(SessionTypeEnum.SESSION3.getSessionName()) && !rowDataMap.get(propertySessionDate).getValue().isEmpty() ) 
		{
			...
			try {
				...
				sessionDateTimeStr = TimeUtil.getDateFromMillis(sessionDateTime.getTimeInMillis(),
		...
		else if(sessionName.equals(SessionTypeEnum.SESSION1.getSessionName()) || sessionName.equals(SessionTypeEnum.SESSION2.getSessionName()))
		{
			sessionDate = rowDataMap.get(propertySessionDate).getValue();
			sessionTime = rowDataMap.get(propertySessionTime).getValue();
			//Calendar sessionDateTime = Calendar.getInstance();
	...
	private boolean isSession3DateValid(Map<String, NetValenceExcelCellVO> rowDataMap, XLSEngine xlsEngine, ApplicationProperties propInstance,
			String sessionDateProperty,String className)
	{
		if (className == "9" || className == "10") {
			String sessionDate = rowDataMap.get(sessionDateProperty).getValue().trim();
			if (null == sessionDate || sessionDate.isEmpty()) {
	...
	private boolean isSession3TimeValid(Map<String, NetValenceExcelCellVO> rowDataMap, XLSEngine xlsEngine, ApplicationProperties propInstance,
			List<String> timeList, String sessionTime,String className)
	{
		if (className == "9" || className == "10") {
			String time = rowDataMap.get(sessionTime).getValue();
			if (null == time || time.isEmpty()) {
	...
	private boolean isCityValid(Map<String, NetValenceExcelCellVO> rowDataMap, XLSEngine xlsEngine, ApplicationProperties propInstance, List<String> cityList)
	{
		...
		if (null != cityList && cityList.contains(city))
		{
			return true;
		}

...
public class StudentWithInterest extends BaseAction{
	...
	public String execute() throws Exception
	{
		...
		ChanakyaStudentDetailsDTO studentDTO = new ChanakyaStudentDetailsDTO();
			...
			if (hearderResult.equalsIgnoreCase("success")) {
				...
				if(Data!=null && !Data.equals("") && !Data.equals("error"))
				{
					...
					if(jsonDataObject.has("testresult") && jsonDataObject.has("studentid") )
					{
						...
						{	
								studentDTO.setCHNKID(jsonDataObject.getString("studentid"));
							...
							ChanakyaStudentDetailsDTO resultDHStudentDTO = service.studentCreation(studentDTO);
							...
							if(jsonDataTestObject!=null && resultDHStudentDTO.getId()!=0)
							{
								...
								List<ChanakyaStudentCGTDTO> studentcgtList = new ArrayList<ChanakyaStudentCGTDTO>();
								
								
								for(Iterator iterator = jsonDataTestObject.keySet().iterator(); iterator.hasNext();) 
								{
									ChanakyaStudentCGTDTO studentcgtDTO = new ChanakyaStudentCGTDTO();
							...
							{
								Status="202";
								Message="Chanakya student Id found, can not upload duplicate record";
								return SUCCESS;

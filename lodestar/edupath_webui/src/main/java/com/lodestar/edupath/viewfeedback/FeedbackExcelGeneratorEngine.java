...
public class FeedbackExcelGeneratorEngine
{
	private static final Logger	OUT	= LoggerFactory.getLogger(FeedbackExcelGeneratorEngine.class);
	...
	public File reportExcel(List<StudentDetailsDTO> reportList, Map<Integer, List<String>> headerMap, String destFilePath, String destinationPath, boolean isAdmin)
	{
		createWorkBook("Feedback_Report_Details");
		createCaptionCell("Feedback Report", headerMap.get(1));
		...
		if (null != reportList && reportList.size() > 0 && !reportList.isEmpty())
		{
			...
			{
				if (studentId != studentDetailsDTO.getId())
				{
					columnIndex = 1;
					createRow();
					//Start Sasedeve edited by Mrutyunjaya on date 08-09-2017
					
					setCellValuesForRow(studentDetailsDTO.getFeedbackdate(), cellStyle, columnIndex++);
					...
					if (isAdmin)
					{
						setCellValuesForRow(studentDetailsDTO.getFacilitatorLoginId(), cellStyle, columnIndex++);
						setCellValuesForRow(studentDetailsDTO.getFacilitatorName(), cellStyle, columnIndex++);
					}
					...
					setCellValuesForRow(studentDetailsDTO.getGender(), cellStyle, columnIndex++);
				}

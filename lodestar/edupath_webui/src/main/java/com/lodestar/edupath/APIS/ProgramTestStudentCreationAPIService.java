...
public class ProgramTestStudentCreationAPIService {
	
	
	private static final Logger OUT = LoggerFactory.getLogger(ProgramTestStudentCreationAPIService.class);
...
//			}
		...
		if (isTrial) {
			...
			if(studentDetailsDTO.getSource().equals(PROGRAMTEST.STREAMSELECTOR.getSource().trim()))
			{
				tempalteName = NotificationConstant.MessageTemplateNameAndType.StreamSelector.name();
				tempalteNameParent = NotificationConstant.MessageTemplateNameAndType.StreamSelector.name();
			}
			else if(studentDetailsDTO.getSource().equals(PROGRAMTEST.ENGINEERINGBRANCHSELECTOR.getSource().trim()))
			{
				tempalteName = NotificationConstant.MessageTemplateNameAndType.EngineeringBranchSelector.name();
				tempalteNameParent = NotificationConstant.MessageTemplateNameAndType.EngineeringBranchSelector.name();
			}
			else if(studentDetailsDTO.getSource().equals(PROGRAMTEST.CAREERDEGREEDISCOVERY.getSource().trim()))
			{
				tempalteName = NotificationConstant.MessageTemplateNameAndType.CareerDegreeDiscovery.name();
				tempalteNameParent = NotificationConstant.MessageTemplateNameAndType.CareerDegreeDiscovery.name();
			}
			else if(studentDetailsDTO.getSource().equals(PROGRAMTEST.CAREERFITMENT.getSource().trim()))
			{
				tempalteName = NotificationConstant.MessageTemplateNameAndType.CareerFitment.name();
				tempalteNameParent = NotificationConstant.MessageTemplateNameAndType.CareerFitment.name();
			}
		}

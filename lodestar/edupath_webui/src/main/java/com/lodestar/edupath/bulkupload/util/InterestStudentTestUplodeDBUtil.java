...
public class InterestStudentTestUplodeDBUtil {
	
	private static final Logger		OUT	 = LoggerFactory.getLogger(InterestStudentTestUplodeDBUtil.class);
	...
	public EActionStatus persistExcelObjects(XLSEngine xlsEngine,Map<String, NetValenceExcelCellVO> rowDataMap,String newinterestLoginID,
			String loggedInUserName) throws Exception
	{
		...
		try
		{
				...
				if (addOrUpdateDetails(rowDataMap, xlsEngine, propInstance, userLoginIdNIdMap, studentCGTList, loggedInUserName, studentCGTDAO,newinterestLoginID))
				{
			...
			if (successfull == 1)
			{
				...
				for (Map.Entry<String, StudentDetailsDTO> stuentMap : userLoginIdNIdMap.entrySet())
				{
					...
					if (StudentTypeEnum.TRIAL == studentDetailsDTO.getStudentType() && studentDetailsDTO.getLoginId().trim().toLowerCase().equals(newinterestLoginID.toLowerCase().trim()))
					{
						interestService.sendMailForTrialStudentBulkUpload(-1, roleDTO.getRoleTypeId(), studentDetailsDTO);
					}
				}
			}
	...
	private boolean addOrUpdateDetails(Map<String, NetValenceExcelCellVO> rowDataMap, XLSEngine xlsEngine, ApplicationProperties propInstance,
			Map<String, StudentDetailsDTO> userLoginIdNIdMap, List<String> studentCGTList, String loggedInUserName, StudentCGTDAO studentCGTDAO,String newinterestLoginID) throws Exception
	{

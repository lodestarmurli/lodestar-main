...
public class SIATTestLink extends BaseAction{
	private static final long	serialVersionUID	= 1L;
	private static final Logger	OUT					= LoggerFactory.getLogger(SIATTestLink.class);
	...
	{
		OUT.debug("Inside SIATTestLink class : execute method");
		try
		{
			String id=null;
	...
	{
		OUT.debug("Inside SIATTestLink class : insert method");
		try
		{
			InterestService		service=new InterestService();
			if(answerList!=null)
	...
	{
		OUT.debug("Inside SIATTestLink class : insertAjax method");
		try
		{
			InterestService		service=new InterestService();

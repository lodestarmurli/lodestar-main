...
public class InterestResultReportService {
	...
	public ChanakyaStudentDetailsDTO getStudentByCHNKID(String id)
	{
		...
		ChanakyaStudentDetailsDTO studentDTO =null;
		try {
			dhstudentDTO.setCHNKID(id.trim());
			studentDTO = dhStudentDAO.getChanakyaStudentByCHNKID(session, dhstudentDTO);
			session.commit();
		} catch (Exception e) {
	...
	public List<OccupationDTO> getOccupationlist(ChanakyaStudentDetailsDTO studentDTO) throws Exception
	{
...
//					allocclist=null;
	...
	public int insertORUpdateStudentOcc(ChanakyaStudentOccupationDTO studentOccDTO) throws Exception
	{
		session = MyBatisManager.getInstance().getSession();
		int id = 0;
		try

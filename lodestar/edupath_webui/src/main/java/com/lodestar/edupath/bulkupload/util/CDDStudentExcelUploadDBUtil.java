...
import com.lodestar.edupath.APIS.ProgramTestStudentCreationAPIService;
...
public class CDDStudentExcelUploadDBUtil
{
	private static final Logger	OUT					= LoggerFactory.getLogger(CDDStudentExcelUploadDBUtil.class);
	public static final int		excelNumberOfRows	= 1000;
	...
	public List<NetValenceExcelRowObject> prepareTemplate()
	{
		...
		try
		{
			setColumnHeadersLabel();
			  ...
			  List<String> genderList = new ArrayList<String>(); setGenderList(genderList);
			  
			/*
			 * List<String> cityList = new ArrayList<String>(); setCityList(cityList, null);
			 * 
			 * List<String> schoolList = new ArrayList<String>(); setSchoolList(schoolList,
			 * null);
			 */
			 ...
			 * 
			 * List<String> timeList = new ArrayList<String>(); setTimeList(timeList);
			 * 
			 * 
			 * //Start Sasedeve Edited By Mrutyunjaya on Date 13-10-2017
			 * 
			 * timeList.remove("9:00"); timeList.remove("9:15"); timeList.remove("9:30");
			 * timeList.remove("9:45");
			 * 
			 * 
			 * List<String> valueList = new ArrayList<String>(); setVenueList(valueList);
			 * 
			 * 
			 * 
			...
			for (int i = 0; i < excelNumberOfRows; i++)
			{
				...
				/*
				 * // city list cellVO = new NetValenceExcelCellVO(); cellVO.setValue("");
				 * cellVO.setCustomListColumn(true); excelRowObject.addRowItem(city, cellVO);
				 * 
				 * // school list cellVO = new NetValenceExcelCellVO(); cellVO.setValue("");
				 * cellVO.setCustomListColumn(true); excelRowObject.addRowItem(school, cellVO);
				 */
				// classList
				cellVO = new NetValenceExcelCellVO();
				...
				/*
				 * // section cellVO = new NetValenceExcelCellVO(); cellVO.setValue("");
				 * excelRowObject.addRowItem(section, cellVO);
				 * 
				 * // facilitator list cellVO = new NetValenceExcelCellVO();
				 * cellVO.setValue(""); cellVO.setCustomListColumn(true);
				 ...
				 * 
				 * // session1Time list cellVO = new NetValenceExcelCellVO();
				 * cellVO.setValueList(timeList); excelRowObject.addRowItem(session1Time,
				 * cellVO);
				 * 
				 * // session2Time list cellVO = new NetValenceExcelCellVO();
				 * cellVO.setValueList(timeList); excelRowObject.addRowItem(session2Time,
				 * cellVO);
				 * 
				 * // session3Time list cellVO = new NetValenceExcelCellVO();
				 * cellVO.setValueList(timeList); excelRowObject.addRowItem(session3Time,
				 * cellVO);
				 * 
				 * // set Venue List cellVO = new NetValenceExcelCellVO();
				 * cellVO.setValueList(valueList); excelRowObject.addRowItem(venue, cellVO);
				 */
			...
			List<SchoolDTO> schoolList, List<ClassDTO> calssList, List<FacilitatorDetailsDTO> facilitatorList, String loggedInUserName)
	{
		...
		for (NetValenceExcelRowObject excelRowObject : excelRowObjList)
		{
			...
			totalRecords++;
			if (addDetails(rowDataMap, xlsEngine, propInstance, cityList, schoolList, calssList, facilitatorList, loggedInUserName))
			{
	...
	private boolean addDetails(Map<String, NetValenceExcelCellVO> rowDataMap, XLSEngine xlsEngine, ApplicationProperties propInstance, List<CityDTO> cityList,
			List<SchoolDTO> schoolList, List<ClassDTO> calssList, List<FacilitatorDetailsDTO> facilitatorList, String loggedInUserName)
	{
		...
		NetValenceExcelCellVO PASWD = rowDataMap.get("PASSWORD");
		try
		{
			...
			studentDetailsDTO.setSource("CAREERDEGREEDISCOVERY_TRAIL");
			EActionStatus status = service.insertStudent(studentDetailsDTO);
			if (status == EActionStatus.SUCCESS)
			{
				StringBuilder auditMessage = new StringBuilder();
				auditMessage.append("CDD Student ").append(studentDetailsDTO.getName())
				...
				xlsEngine.setCellValueExternally(LDID.getRowIndex(), LDID.getColumnIndex(), service.LDID);
				xlsEngine.setCellValueExternally(PASWD.getRowIndex(), PASWD.getColumnIndex(), service.PASSWD);
				return true;
			}
		}
		catch (Exception e)
			...
			List<CityDTO> cityList, List<SchoolDTO> schoolList, List<ClassDTO> calssList, List<FacilitatorDetailsDTO> facilitatorList)
	{
		synchronized (this)
		{
			try
			{
				...
				/*
				 * if
				 * (rowDataMap.get(propInstance.getProperty("com.edupath.bulkupload.column.city"
				 * )).getValue().trim() != null &&
				 * !rowDataMap.get(propInstance.getProperty("com.edupath.bulkupload.column.city"
				 * )).getValue().trim().isEmpty()) { for (CityDTO cityDTO : cityList) { if
				 * (cityDTO.getName().equals(rowDataMap.get(propInstance.getProperty(
				 * "com.edupath.bulkupload.column.city")).getValue().trim())) {
				 * studentDetailsDTO.setCityId(cityDTO.getId()); break; } } } if
				 * (rowDataMap.get(propInstance.getProperty(
				 * "com.edupath.bulkupload.column.school")).getValue().trim() != null &&
				 * !rowDataMap.get(propInstance.getProperty(
				 * "com.edupath.bulkupload.column.school")).getValue().trim().isEmpty()) { for
				 * (SchoolDTO schoolDTO : schoolList) { if
				 * (schoolDTO.getName().equals(rowDataMap.get(propInstance.getProperty(
				 * "com.edupath.bulkupload.column.school")).getValue().trim())) {
				 * studentDetailsDTO.setSchoolId(schoolDTO.getId()); break; } } }
				 */
				...
				/*
				 * if (rowDataMap.get(propInstance.getProperty(
				 * "com.edupath.bulkupload.column.computerfacility")).getValue().trim() != null
				 * && !rowDataMap.get(propInstance.getProperty(
				 * "com.edupath.bulkupload.column.computerfacility")).getValue().trim().isEmpty(
				 * )) { String computerfacility = rowDataMap.get(propInstance.getProperty(
				 * "com.edupath.bulkupload.column.computerfacility")).getValue().trim(); if
				 * (computerfacility.equalsIgnoreCase("Yes")) {
				 * studentDetailsDTO.setComputerFacility(true); } else if
				 * (computerfacility.equalsIgnoreCase("No")) {
				 * studentDetailsDTO.setComputerFacility(false); }
				 * 
				 * } if (rowDataMap.get(propInstance.getProperty(
				 * "com.edupath.bulkupload.column.facilitator")).getValue().trim() != null &&
				 * !rowDataMap.get(propInstance.getProperty(
				 * "com.edupath.bulkupload.column.facilitator")).getValue().trim().isEmpty()) {
				 * for (FacilitatorDetailsDTO facilitatorDetailsDTO : facilitatorList) { if
				 * (facilitatorDetailsDTO.getName().equals(
				 * rowDataMap.get(propInstance.getProperty(
				 * "com.edupath.bulkupload.column.facilitator")).getValue().trim())) {
				 * studentDetailsDTO.setFacilitatorId(facilitatorDetailsDTO.getId()); break; } }
				 * } Calendar sessionDateTime;
				 * 
				 * if (rowDataMap.get(propInstance.getProperty(
				 * "com.edupath.bulkupload.column.session1date")).getValue().trim() != null &&
				 * !rowDataMap.get(propInstance.getProperty(
				 * "com.edupath.bulkupload.column.session1date")).getValue().trim().isEmpty() &&
				 * rowDataMap.get(propInstance.getProperty(
				 * "com.edupath.bulkupload.column.session1time")).getValue().trim() != null &&
				 * !rowDataMap.get(propInstance.getProperty(
				 * "com.edupath.bulkupload.column.session1time")).getValue().trim().isEmpty()) {
				 * String session1DateTimeStr = null; try { sessionDateTime =
				 * Calendar.getInstance(); Date date = new
				 * Date(TimeUtil.getDate(rowDataMap.get(propInstance.getProperty(
				 * "com.edupath.bulkupload.column.session1date")).getValue(),
				 * TimeUtil.EXCEL_DATE_FORMAT).getTime()); Date time = new
				 * Date(TimeUtil.getDate(rowDataMap.get(propInstance.getProperty(
				 * "com.edupath.bulkupload.column.session1time")).getValue(),
				 * TimeUtil.EXCEL_DATE_FORMAT).getTime());
				 * 
				 * sessionDateTime.setTime(date); sessionDateTime.add(Calendar.HOUR,
				 * time.getHours()); sessionDateTime.add(Calendar.MINUTE, time.getMinutes());
				 * session1DateTimeStr =
				 * TimeUtil.getDateFromMillis(sessionDateTime.getTimeInMillis(),
				 * TimeUtil.FILTER_DATE_TIME_FORMAT); } catch (Exception e) {
				 * OUT.error(ApplicationConstants.EXCEPTION, e); }
				 * studentDetailsDTO.setSession1DateStr(session1DateTimeStr); }
				 * 
				 * if (rowDataMap.get(propInstance.getProperty(
				 * "com.edupath.bulkupload.column.session2date")).getValue().trim() != null &&
				 * !rowDataMap.get(propInstance.getProperty(
				 * "com.edupath.bulkupload.column.session2date")).getValue().trim().isEmpty() &&
				 * rowDataMap.get(propInstance.getProperty(
				 * "com.edupath.bulkupload.column.session2time")).getValue().trim() != null &&
				 * !rowDataMap.get(propInstance.getProperty(
				 * "com.edupath.bulkupload.column.session2time")).getValue().trim().isEmpty()) {
				 * String session2DateTimeStr = null; try { sessionDateTime =
				 * Calendar.getInstance(); Date date = new
				 * Date(TimeUtil.getDate(rowDataMap.get(propInstance.getProperty(
				 * "com.edupath.bulkupload.column.session2date")).getValue(),
				 * TimeUtil.EXCEL_DATE_FORMAT).getTime()); Date time = new
				 * Date(TimeUtil.getDate(rowDataMap.get(propInstance.getProperty(
				 * "com.edupath.bulkupload.column.session2time")).getValue(),
				 * TimeUtil.EXCEL_DATE_FORMAT).getTime());
				 * 
				 * sessionDateTime.setTime(date); sessionDateTime.add(Calendar.HOUR,
				 * time.getHours()); sessionDateTime.add(Calendar.MINUTE, time.getMinutes());
				 ...
				 * 
				 * if (rowDataMap.get(propInstance.getProperty(
				 * "com.edupath.bulkupload.column.session3date")).getValue().trim() != null &&
				 * !rowDataMap.get(propInstance.getProperty(
				 * "com.edupath.bulkupload.column.session3date")).getValue().trim().isEmpty() &&
				 * rowDataMap.get(propInstance.getProperty(
				 * "com.edupath.bulkupload.column.session3time")).getValue().trim() != null &&
				 * !rowDataMap.get(propInstance.getProperty(
				 * "com.edupath.bulkupload.column.session3time")).getValue().trim().isEmpty()) {
				 * String session3DateTimeStr = null; try { sessionDateTime =
				 * Calendar.getInstance(); Date date = new
				 * Date(TimeUtil.getDate(rowDataMap.get(propInstance.getProperty(
				 * "com.edupath.bulkupload.column.session3date")).getValue(),
				 * TimeUtil.EXCEL_DATE_FORMAT).getTime()); Date time = new
				 * Date(TimeUtil.getDate(rowDataMap.get(propInstance.getProperty(
				 * "com.edupath.bulkupload.column.session3time")).getValue(),
				 * TimeUtil.EXCEL_DATE_FORMAT).getTime());
				 * 
				 * sessionDateTime.setTime(date); sessionDateTime.add(Calendar.HOUR,
				 * time.getHours()); sessionDateTime.add(Calendar.MINUTE, time.getMinutes());
				 ...
				 * 
				 * if (rowDataMap.get(propInstance.getProperty(
				 * "com.edupath.bulkupload.column.venue")).getValue().trim() != null &&
				 * !rowDataMap.get(propInstance.getProperty(
				 * "com.edupath.bulkupload.column.venue")).getValue().trim().isEmpty()) { String
				 * venue =
				 * rowDataMap.get(propInstance.getProperty("com.edupath.bulkupload.column.venue"
				 * )).getValue().trim(); if
				 * (venue.equalsIgnoreCase(VenueTypeEnum.OTHER.getValue())) { String otherVenue
				 * = rowDataMap.get(propInstance.getProperty(
				 * "com.edupath.bulkupload.column.venue.other")).getValue().trim();
				 * studentDetailsDTO.setVenue(otherVenue); } else if
				 * (venue.equalsIgnoreCase("AT SCHOOL")) {
				 * studentDetailsDTO.setVenue(VenueTypeEnum.ATSCHOOL.toString()); } else if
				 * (venue.equalsIgnoreCase(VenueTypeEnum.ATOFFICE.getValue())) {
				...
				/*
				 * if (rowDataMap.get(propInstance.getProperty(
				 * "com.edupath.bulkupload.column.student.section")).getValue() != null) {
				 * studentDetailsDTO.setSection(rowDataMap.get(propInstance.getProperty(
				 * "com.edupath.bulkupload.column.student.section")).getValue());
				 * 
				 * }
				 * 
				 * if (rowDataMap.get(propInstance.getProperty(
				 * "com.edupath.bulkupload.column.student.test")).getValue() != null &&
				 * !rowDataMap.get(propInstance.getProperty(
				 * "com.edupath.bulkupload.column.student.test")).getValue().trim().isEmpty()) {
				 * studentDetailsDTO.setTestTaken(rowDataMap.get(propInstance.getProperty(
				 * "com.edupath.bulkupload.column.student.test")).getValue());
				 * 
				 * if (StudentTypeEnum.StudentTestTakenEnum.YES.name().equalsIgnoreCase(
				 * studentDetailsDTO.getTestTaken())) {
				 ...
				 * 
				 * if (rowDataMap.get(propInstance.getProperty(
				 * "com.edupath.bulkupload.column.agreedAmount")).getValue() != null &&
				 * !rowDataMap.get(propInstance.getProperty(
				 * "com.edupath.bulkupload.column.agreedAmount")).getValue().trim().isEmpty()) {
				 * studentDetailsDTO.setAgreedAmount(Double.parseDouble(rowDataMap.get(
				 * propInstance.getProperty("com.edupath.bulkupload.column.agreedAmount")).
				 * getValue()));
				 * 
				 * } if (rowDataMap.get(propInstance.getProperty(
				 * "com.edupath.bulkupload.column.paidAmount")).getValue() != null &&
				 * !rowDataMap.get(propInstance.getProperty(
				 * "com.edupath.bulkupload.column.paidAmount")).getValue().trim().isEmpty()) {
				 * studentDetailsDTO.setPaidAmount(Double.parseDouble(rowDataMap.get(
				 * propInstance.getProperty("com.edupath.bulkupload.column.paidAmount")).
				 * getValue()));
				 * 
				 * 
				 * }
				...
				/*
				 * if (rowDataMap.get(propInstance.getProperty(
				 * "com.edupath.bulkupload.column.mothername")).getValue().trim() != null &&
				 * !rowDataMap.get(propInstance.getProperty(
				 * "com.edupath.bulkupload.column.mothername")).getValue().trim().isEmpty()) {
				 * 
				 * studentDetailsDTO.setMotherName(rowDataMap.get(propInstance.getProperty(
				 * "com.edupath.bulkupload.column.mothername")).getValue()); } if
				 * (rowDataMap.get(propInstance.getProperty(
				 * "com.edupath.bulkupload.column.mothercontact")).getValue().trim() != null &&
				 * !rowDataMap.get(propInstance.getProperty(
				 * "com.edupath.bulkupload.column.mothercontact")).getValue().trim().isEmpty())
				 * {
				 * 
				 * studentDetailsDTO.setMothercontactno(rowDataMap.get(propInstance.getProperty(
				 * "com.edupath.bulkupload.column.mothercontact")).getValue()); } if
				 * (rowDataMap.get(propInstance.getProperty(
				 * "com.edupath.bulkupload.column.motheremail")).getValue().trim() != null &&
				 * !rowDataMap.get(propInstance.getProperty(
				 * "com.edupath.bulkupload.column.motheremail")).getValue().trim().isEmpty()) {
				 * 
				 * studentDetailsDTO.setMotheremailId(rowDataMap.get(propInstance.getProperty(
				 * "com.edupath.bulkupload.column.motheremail")).getValue()); }
				 */
			}
			catch (Exception e)
			{
	...
	{
		return new StudentDetailService().sessionValidGap();
	}

	...
	public String execute() {
		...
		if (token != null || token != "") {
			...
			if (hearderResult.equalsIgnoreCase("success")) {
				...
				// jsonDataObject1 :{"StudentId":"4","Occupation":"193","date":"Fri Apr 03
				// 15:08:15 IST 2020"}

				try {

					if (jsonDataObject.has("StudentId") && jsonDataObject.has("Stage")
							&& jsonDataObject.has("date")) {
					...
					ChanakyaStudentOccupationDTO studentoccDTO = studentOccDAO
							.getChanakyaStudentOccupationbyToken(token);
					if (studentoccDTO.getStudentId() == studentDTO.getId()) {
						docDTO = docDAO.getDocumentByOccId(1);

...
public class InterestResultReport extends BaseAction{
	...
	private ClientStudentDAO studentDAO = new ClientStudentDAO();
		...
		try
		{
			...
			if (!hearderResult_partnerName.equalsIgnoreCase("error") && !hearderResult_partnerName.equalsIgnoreCase("errorMethod")) 
			{
				...
				if(id!=null && !getId().equals("") )
				{
					...
					ClientStudentDetailsDTO studentDTO = service.getStudentByDHID(id.trim(), hearderResult_partnerName);
					...
					if (studentDTO != null) 
					{
						...
						ClientStudentOccupationDTO studentOccDTO = new ClientStudentOccupationDTO();
						...
						or_report1 = globalDTO.getPropertyValue()+ ApplicationProperties.getInstance().getProperty("com.Client.OR.download.url")+token1;
						or_report2 = globalDTO.getPropertyValue()+ApplicationProperties.getInstance().getProperty("com.Client.OR.download.url")+token2;
						interestreportlink =globalDTO.getPropertyValue()+ApplicationProperties.getInstance().getProperty("com.Client.pdf.download.url")+ pdfToken;
				...
				else
				{
					...
					interestreportlink="";
					Status="404";
					Message="Client student ID null";
				}

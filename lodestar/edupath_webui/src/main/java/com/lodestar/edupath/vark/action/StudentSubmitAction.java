...
import com.lodestar.edupath.vark.service.StudentRegistrationService;
import com.opensymphony.xwork2.ModelDriven;

public class StudentSubmitAction extends BaseAction implements ModelDriven<StudentDetailsDTO>
{
	private static final long			serialVersionUID	= 4228877756086138530L;
	private static Logger				OUT					= LoggerFactory.getLogger(StudentSubmitAction.class);
	...
	//Start SASEDEVE Edited by Mrutyunjaya On Date 24-05-2017
	...
	//END SASEDEVE Edited by Mrutyunjaya On Date 24-05-2017

	public StudentSubmitAction()
	{
...
//			}
			...
			studentDTO.setSource("VARK_TRAIL");
			fieldRequired();
...
//			insertAudit(auditMessage.toString(), userSessionObject);
	...
	private boolean checkDTO()
	{
		...
		if (null == studentDTO.getSession1DateStr() || null == studentDTO.getSession2DateStr() || null == studentDTO.getSession3DateStr())
		{
			return false;
		}
		return true;
	...
	private String checkFacilitatorDependency(StudentDetailsDTO studentDTO)
	{
		...
		// List<SessionScheduleDetailsDTO> seList = null;
		try
		{
			int session1Count = -1;
			int session2Count = -1;
			...
			session1Count = new SessionScheduleDetailsDAO().getcountBySessionByDate(studentDTO.getId(), studentDTO.getFacilitatorId(),
					TimeUtil.getDate(studentDTO.getSession1DateStr(), TimeUtil.FILTER_DATE_TIME_FORMAT), 1);

			session2Count = new SessionScheduleDetailsDAO().getcountBySessionByDate(studentDTO.getId(), studentDTO.getFacilitatorId(),
					TimeUtil.getDate(studentDTO.getSession2DateStr(), TimeUtil.FILTER_DATE_TIME_FORMAT), 2);
			if((studentDTO.getClassId()==1 ||studentDTO.getClassId()==2) && (studentDTO.getSession3DateStr()!=null || !studentDTO.getSession3DateStr().equals("")))
			{
				session3Count = new SessionScheduleDetailsDAO().getcountBySessionByDate(studentDTO.getId(), studentDTO.getFacilitatorId(),
			...
			{
				List<SessionScheduleDetailsDTO> getSessionDetaillist = new SessionScheduleDetailsDAO().getDetailsByFacilitatorId(studentDTO.getFacilitatorId());
	...
	private boolean isValidSessionDateTime(Date sessionDate, int count)
	{
		...
		try
		{
			...
			{
				case 1:
					iterateSessionStartTime.setTime(TimeUtil.getDate(studentDTO.getSession1DateStr(), TimeUtil.FILTER_DATE_TIME_FORMAT));
					iterateSessionStopTime.setTime(TimeUtil.getDate(studentDTO.getSession1DateStr(), TimeUtil.FILTER_DATE_TIME_FORMAT));
					iterateSessionStopTime.add(Calendar.HOUR, ApplicationConstants.SESSION_HOURS);
					break;
				case 2:
					iterateSessionStartTime.setTime(TimeUtil.getDate(studentDTO.getSession2DateStr(), TimeUtil.FILTER_DATE_TIME_FORMAT));
					if(studentDTO.getSession3DateStr()!=null) {
						iterateSessionStopTime.setTime(TimeUtil.getDate(studentDTO.getSession3DateStr(), TimeUtil.FILTER_DATE_TIME_FORMAT));
					}
					iterateSessionStopTime.add(Calendar.HOUR, ApplicationConstants.SESSION_HOURS);
					break;
				case 3:
					if(studentDTO.getSession3DateStr()!=null) {
						iterateSessionStartTime.setTime(TimeUtil.getDate(studentDTO.getSession3DateStr(), TimeUtil.FILTER_DATE_TIME_FORMAT));
						iterateSessionStopTime.setTime(TimeUtil.getDate(studentDTO.getSession3DateStr(), TimeUtil.FILTER_DATE_TIME_FORMAT));
						iterateSessionStopTime.add(Calendar.HOUR, ApplicationConstants.SESSION_HOURS);
					}
					break;
			...
			if (null != sessionDate)
			{
				...
				if (iterateSessionStartTime.getTime().equals(startTime.getTime()) || iterateSessionStartTime.getTime().after(startTime.getTime())
	...
	private void fieldRequired() throws Exception
	{
		...
		// setSessionTimeGap(service.sessionValidGap());
		
		//Start SASEDEVE Edited by Mrutyunjaya On Date 24-05-2017
		
		
		Map<Integer, ArrayList<Map<String, String>>> VenueAdressMap = null;
		//Map<String, String> VenueAdressDetailMap = null;
		...
		//String schoolCityMapString=mapper.writeValueAsString(schoolCityMap);
...
//	{
//		OUT.debug("update exist student Record with name", studentDTO.getName());
////		UserSessionObject userSessionObject = getUserSessionObject();
////		if (null == userSessionObject)
////		{
////			return "SessionExpired";
...
//		{
//			studentDTO.setId(studentId.intValue());
//		}
...
//				{
//					addActionError("can not be updated student, due to Exception.");
//				}
//				else
//				{
...
//			}
//		}
//		EActionStatus status = service.updateStudent(studentDTO, schoolList, userSessionObject);
//		if (status == EActionStatus.FAILURE)
//		{
//			addActionError("can't add due to Exception");
//			return ERROR;
//		}
//		StringBuilder auditMessage = new StringBuilder();
//		auditMessage.append("Student ").append(studentDTO.getName()).append(" details has been changed successfully and updated");
//		insertAudit(auditMessage.toString(), userSessionObject);
//		addActionMessage(auditMessage.toString());
//		OUT.debug("Done update exist student Record with name", studentDTO.getName());
//		return SUCCESS;
//
//	}

...
public class StudentAptitudeTestAction extends BaseAction{
	...
	{
		OUT.debug("Excute the StudentAptitudeTestAction Summary Action Of======================= {}",StudentAptitudeTestAction.class);
		...
		try
		{
			...
			if (!hearderResult_partnerName.equalsIgnoreCase("error") && !hearderResult_partnerName.equalsIgnoreCase("errorMethod")) 
			{
				...
				if(Data!=null && !Data.equals("") && !Data.equals("error"))
				{
					...
					{
						if(jsonDataObject.getString("studentid").trim() != null && !jsonDataObject.getString("studentid").trim().equalsIgnoreCase(" ") && !jsonDataObject.getString("studentid").trim().equalsIgnoreCase(""))
						{	
							String DHID = jsonDataObject.getString("studentid");
							studentDTO = service.getStudentAptitudeByDHID(DHID,hearderResult_partnerName);
							if(studentDTO != null )
							{			
								if(studentDTO.getAptitudeComplete() == null || studentDTO.getAptitudeComplete().equals("") )		
								{
									...
									if(jsonDataObject.getString("studentType").trim() != null && !jsonDataObject.getString("studentType").trim().equalsIgnoreCase(" ")  && !jsonDataObject.getString("studentType").trim().equalsIgnoreCase(""))
									{
								...
								{
									Status="202";
									Message="Test results for student id " + studentDTO.getClientID() +" exists";
									return SUCCESS;		
								}
							...
							{
								Status="404";
								Message="Client student Id not found, can not upload test results";
								return SUCCESS;	
							}

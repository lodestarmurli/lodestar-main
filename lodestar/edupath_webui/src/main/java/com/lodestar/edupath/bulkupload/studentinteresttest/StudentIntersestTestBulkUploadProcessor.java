...
import com.lodestar.edupath.bulkupload.util.InterestStudentTestUplodeDBUtil;
...
public class StudentIntersestTestBulkUploadProcessor {
	

	private static final Logger						OUT				= LoggerFactory.getLogger(StudentIntersestTestBulkUploadProcessor.class);

	private static final StudentIntersestTestBulkUploadProcessor	instance		= new StudentIntersestTestBulkUploadProcessor();
	...
	public static StudentIntersestTestBulkUploadProcessor getInstance()
	{
		return instance;
	}
			...
			int loggedInUserId, String loggedinUserLoginId)
	{
		...
		try
		{
			...
			Callable<BulkUploadMessage> callable = new StudentInterstBulkUploadJob(file, fileName, xmlFilePath, loggedInUserFullName, moduleName, loggedInUserId,
					loggedinUserLoginId);
	...
	private class StudentInterstBulkUploadJob implements Callable<BulkUploadMessage>
	{
		...
		public StudentInterstBulkUploadJob(File uploadFile, String fileName, String xmlFilePath, String loggedInUserName, String moduleName, int loggedInUserId,
		...
		public BulkUploadMessage call()
		{
			...
			try
			{
				...
				System.out.println(" vyankatesh =================================================="+excelRowObjList);
				...
				isValidationSuccess = new StudentInterestTestValidator().validate(xlsEngine, excelRowObjList, cityList, schoolList, classList, facilitatorList);
				...
				EActionStatus persistStatus = new StudentInterestTestExcelUploadDBUtil().persistExcelObjects(xlsEngine, excelRowObjList, cityList, schoolList, classList, facilitatorList,loggedInUserName);
				...
				if (uploadedFile != null)
				{
					...
					if(returnMsg== BulkUploadMessage.SUCCESS)
					{
						...
						if (null != globalDTO.getPropertyValue() && !globalDTO.getPropertyValue().isEmpty())
						{
							...
							if(newFolder!=null)
							{
								File processedFile = new File(currentDir + newFolder +"/"+ "Uploaded_Student_processed" + ".xls");
								try {
									file = xlsEngine.getErrorWorkbook(processedFile);
									
									
									try {
										PasswordGeneratorService.sendNewNotificationToAdmin(null, "Admin", "guidance@lodestar.guru", 2,
												NotificationConstant.MessageTemplateNameAndType.Admin_EMAIL_BULK_UPLOAD_RESULT.name(), newFolder, "Admin");
									} catch (Exception e) {
										// TODO Auto-generated catch block
										//e.printStackTrace();
									}

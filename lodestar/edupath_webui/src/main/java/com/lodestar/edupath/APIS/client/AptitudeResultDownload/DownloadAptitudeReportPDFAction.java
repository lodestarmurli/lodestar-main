...
import com.lodestar.edupath.APIS.client.InterestResultReportService;
import com.lodestar.edupath.APIS.client.InterestResultUtilService; 
import com.lodestar.edupath.APIS.client.Util;
import com.lodestar.edupath.base.BaseAction;
import com.lodestar.edupath.dataaccessobject.dao.APIS.client.ClientEdupathDAO;
...
public class DownloadAptitudeReportPDFAction  extends BaseAction{
	...
	public String execute()
	{
		...
		if(token!=null || token!="")
		{
			...
			if (!hearderResult.equalsIgnoreCase("error") && !hearderResult.equalsIgnoreCase("errorMethod"))  {
				...
				try 
				{
					...
					if(jsonDataObject.has("studentType"))
					{
						...
						List<ClientEdupathDTO> edupathlist= ClientEdupathDAO.getDHEdupathbyOccId(occupationIds);
						for(OccupationDTO occ:occlistwithfitment)
						{
							for(ClientEdupathDTO edu:edupathlist)
							{
								if(occ.getId()==edu.getOccupationId())
								{
//									edu.setOccName(occ.getName());
//									edu.setDhPriority(occ.getDhPriority());
									occ.setClienteduDTO(edu);
								}
							}
						}
						...
						for(ClientEdupathDTO dhedupath : edupathlist)
						{
						...
						List<ClientEntExamDTO> entExamlist = ClientEdupathDAO.getexamdescription(examList);
...
class EdupathPrioritySorter implements Comparator<ClientEdupathDTO>
{
	private static final Logger	OUT	= LoggerFactory.getLogger(EdupathPrioritySorter.class);
	public int compare(ClientEdupathDTO e1, ClientEdupathDTO e2)
	{			

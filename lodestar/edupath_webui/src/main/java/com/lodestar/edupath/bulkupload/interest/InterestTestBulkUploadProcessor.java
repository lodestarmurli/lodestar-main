...
public class InterestTestBulkUploadProcessor
{
	private static final Logger								OUT				= LoggerFactory.getLogger(InterestTestBulkUploadProcessor.class);

	private static final InterestTestBulkUploadProcessor	instance		= new InterestTestBulkUploadProcessor();
	...
	public static InterestTestBulkUploadProcessor getInstance()
	{
		return instance;
	}
			...
			int loggedInUserId, String loggedinUserLoginId)
	{
		...
		try
		{
			...
			Callable<BulkUploadMessage> callable = new InterestTestBulkUploadJob(file, fileName, xmlFilePath, loggedInUserFullName, moduleName, loggedInUserId,
					loggedinUserLoginId);
	...
	private class InterestTestBulkUploadJob implements Callable<BulkUploadMessage>
	{
		File	uploadedFile;
		...
		public InterestTestBulkUploadJob(File uploadFile, String fileName, String xmlFilePath, String loggedInUserName, String moduleName, int loggedInUserId,
		...
		public BulkUploadMessage call()
		{
			...
			try
			{
				...
				if (null == userLoginIdNIdMap || userLoginIdNIdMap.isEmpty())
				{
					OUT.warn("-------------------- Students Details Not Founded in db ----------------------");
				...
				isValidationSuccess = new InterestTestExcelValidator().validate(xlsEngine, excelRowObjList, userLoginIdNIdMap);
				...
				EActionStatus persistStatus = new InterestTestExcelUploadDBUtil().persistExcelObjects(xlsEngine, excelRowObjList, userLoginIdNIdMap,
						loggedInUserName);

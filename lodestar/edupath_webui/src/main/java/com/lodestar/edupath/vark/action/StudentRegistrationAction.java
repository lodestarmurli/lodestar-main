...
public class StudentRegistrationAction extends BaseAction implements ModelDriven<StudentBean>
{

	private static final long			serialVersionUID	= 6090415257988873149L;
	private static Logger				OUT					= LoggerFactory.getLogger(StudentRegistrationAction.class);
	...
	{
		OUT.debug("bharath inside vark StudentRegistrationAction ");
		fieldRequired();
...
//		timeList = service.getTimeList();
		//List<VenueDetailDTO> venuelist = new StudentDetailsDAO().getAllvenueDetail();
		
		//Start SASEDEVE Edited by Vynketesh On Date 24-05-2017
...
//		facilitatorCityjsonObject.put("facilitatorcityMap", facilitatorCityMap);
		...
		//VenueAdressDetailMap = service.getallAddressvenuedetail(venuelist);
		//venuejsonObject.put("VenueAdressDetailMap", schoolList);
		
		
		//END SASEDEVE Edited by Vynketesh On Date 24-05-2017
		
		
		
		globalSettingMap = new HashMap<String, String>();
		service.getGlobalSettingMap(globalSettingMap);
		// sessionTimeGap = service.sessionValidGap();
	...
	{
		OUT.debug("StudentDetailsAction : inside openSession method");
		try
		{
			OUT.debug("studentId : {} and userId : {} and openSessionType : {}", studentId, studentDTO.getUserId(), openSessionType);
			service.modifyStudentSessionByOpenSession(studentId, studentDTO.getUserId(), openSessionType, getUserSessionObject().getLoginId());
		}
		catch (Exception e)
		{

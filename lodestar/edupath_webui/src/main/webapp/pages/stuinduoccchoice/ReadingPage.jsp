		...
		<div class="row rpheadingbar">
			...
			<div class="col-md-4">
				<div class="pull-right actionitems">
					<div id="hightlight">
						<img src="${pageContext.request.contextPath}/images/highlight.gif"> Highlight
					</div>
					<div id="showNotes">
						<img src="${pageContext.request.contextPath}/images/students-notes-reading-interface.gif"> My Notes
					</div>
					<div id="readerclose" style="border-left: none;">
						<i class="fa fa-times-circle-o fa-2x" style="color: #8ac5e5"></i>
					</div>
				</div>
			</div>
		</div>
		<div class="row">
			<div class="col-md-7 rpsubmenu">
				<div class="row" id="relatedOccDiv" style="display: none;">
					<div class="pull-left rprelatedocc">
						Related Occupation:
					</div>
					<div class="col-md-9" id="relatedOcc">
					</div>
				</div>&nbsp;
			</div>
			<div class="col-md-5 rpsubmenu subactionitems pull-right">
				<div id="addSummartToNotes" style="display: none; width:70%; padding-left:10px; white-space:none;" onclick="fnCopySummaryToNotes()">
					<div class="subaction_el">
						<img src="${pageContext.request.contextPath}/images/student-reading-add.gif">
					</div>
					<div class="subaction_el">
						Add Report Summary to My Notes
					</div>
				</div>
				<div style="white-space: nowrap;">
					<div id="addShort" onclick="fnAddtoShortList()" style="display:none">
						<div class="subaction_el">
							<img src="${pageContext.request.contextPath}/images/student-reading-add.gif">
						</div>
						<div class="subaction_el">
					...
					<div style="display: none;" id="removeShort" onclick="fnDeleteFromShortList()">
						<div class="subaction_el">
							<img src="${pageContext.request.contextPath}/images/student-reading-remove.gif">
						</div>
						<div class="subaction_el">	
...
<script lang=JavaScript> 
	...
	var maxpages = 1;
	var startTimer;
	var docType = window.opener.docType;
	var datastr = window.opener.datastr;
	var fromCart = window.opener.fromCart;
...
/*		viewer = Crocodoc.createViewer($("#boxviewcontent"), {
	...
	{
		console.log("bharath inside fnGetOccAdditionalDetail related=>"+related)
		console.log("bharath fnGetOccAdditionalDetail fromCart=>"+fromCart)
		$.ajax({
			...
			success : function(result) {
				console.log("bharath fnGetOccAdditionalDetail result=>"+result)
				$("#loader_div").hide();
				if (result.status == "SUCCESS") {
					if (result.details.shortListId > 0) {
						console.log("bharath fnGetOccAdditionalDetail if result.details.shortListId=>"+result.details.shortListId)
						$("#addShort").hide();
						$('#addSummartToNotes').css('width', '70%');
						$("#removeShort").show();
						shortListId = result.details.shortListId;
					} else {
						console.log("bharath fnGetOccAdditionalDetail else result.details.shortListId=>"+result.details.shortListId)
						$("#addShort").show();
			...
			error : function(jqXHR, textStatus, errorThrown) {
				console.log("bharath fnGetOccAdditionalDetail error")
				//alert(errorThrown)
			}
		});
	}
	...
	function fnGetDocumentURLNLoad(relatedOcc, local_dataStr) {
		...
		console.log("local_dataStr==>"+local_dataStr)
		$("#relatedOccDiv").hide();
		$("#relatedOcc").children().remove();
		$("#addShort").hide();
		$("#removeShort").hide();
		...
		$("#loader_div").show();
		$.ajax({
					url : "${pageContext.request.contextPath}/myapp/documentURLDocumentAction",
					method : "POST",
					data : local_dataStr,
					dataType : "json",
					success : function(result) {

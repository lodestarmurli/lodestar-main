...
<BODY style=" background-color:#f0f9ff">
  ...
  <p></p>
    ...
    <div class="form-group">
      <label for="inputlg">Parent Email Address <font color="red">*</font></label>
      <input class="form-control input-lg" id="parentEmail" type="text" value="">
    </div>
    
    <div class="form-group">
      <label for="inputlg">Parent Mobile Number <font color="red">*</font></label>
      <input class="form-control input-lg" id="parentno" type="text" value="">
    </div>
       ...
       <div id="sb" class="col-sm-offset-3 col-md-offset-1 col-lg-offset-2 col-sm-7 col-md-10 col-lg-8" >
        <button id="sbb" type="button" onclick="StudentRegistor();" class="btn btn-info btn-lg btn-block">Start test Now</button>
		</div>
      <br/><br/><br/>
...
<s:form name="Studentreg" id="Studentreg" method="post">
   ...
   <input type="hidden" name="srtoken" id="srtoken" value="${token}" />
...
function StudentRegistor()
{
	...
	if(Validatinginput())
	{
		...
		$('#srpno').val($('#pno').val());
		$('#premail').val($('#parentEmail').val());
		$('#prpno').val($('#parentno').val());
		
		
		form.action = "${pageContext.request.contextPath}/APIS/NewTYEProgTestRegistration";
		var options = { 
...
function Validatinginput()
{
	...
	if($('#parentEmail').val()!=null && $('#parentEmail').val().trim()!="")
	{
		 var reg = /^([A-Za-z0-9_\-\.])+\@([A-Za-z0-9_\-\.])+\.([A-Za-z]{2,4})$/;
		
		 if (reg.test($('#parentEmail').val()) == false) 
	       {
			    alert('Invalid Email Address');
			    $('#parentEmail').addClass('has-error has-danger');
				$('#parentEmail').focus();
				return false;
	       }
	...
	if($('#parentno').val()!=null && $('#parentno').val().trim()!="")
	{
		
		var phoneno = /^\d{10}$/; 
		
		if($('#parentno').val().trim().match(phoneno))
		{}
		else
		{
			alert('Invalid Phone Number');
		    $('#parentno').addClass('has-error has-danger');
			$('#parentno').focus();
			return false;
		}
...
function showResponse(responseText, statusText, xhr, $form)
{
	...
	if(responseText.status=="success")
	{
		...
		$('#token').val(responseText.srtoken);
		
		console.log("====>"+responseText.srtoken)
		
		form1.action = "${pageContext.request.contextPath}/Test/StartTYEProgTest";
		 form1.submit();
	}

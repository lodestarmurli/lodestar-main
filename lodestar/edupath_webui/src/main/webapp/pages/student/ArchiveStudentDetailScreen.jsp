...
function goBack() {
	form.action = "${pageContext.request.contextPath}/myapp/ArchiveStudents";
	form.submit();
}
...
function fnSubmit() {
	if (isValidate()) {
		...
		$('#studentId').val(id);
		if (id != '' && id != '0') {
			form.action = "${pageContext.request.contextPath}/myapp/updateStudentArchiveSubmit";
		} else {
			form.action = "${pageContext.request.contextPath}/myapp/insertStudentArchiveSubmit";
		}
		form.submit(); 
	}
}
...
function isSessionValid(session1_date, session1_time, session2_date,
	...
	if (null == session3_date || session3_date == '') {
		$('.session3Date-error').html(
				"session 3: Date is required information");
		return false;
	}
	if (null == session3_time || session3_time == '') {
		$('.session3Date-error').html(

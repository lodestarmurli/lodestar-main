		...
		<div class="portlet box blue">
			<div class="portlet-title">
				<div class="caption">
					...
					<s:text name="com.DailyHunt.scheduleAppointmentSummary.title"></s:text>
				</div>
			</div>
					...
					<table
						...
						<thead>
							<tr>
								...
								<th><s:text name="com.DailyHunt.scheduleAppointmentSummary.name" /></th>
								<th><s:text name="com.DailyHunt.scheduleAppointmentSummary.email" /></th>
								<th><s:text name="com.DailyHunt.scheduleAppointmentSummary.contact" /></th>
								<th><s:text name="com.DailyHunt.scheduleAppointmentSummary.session1Date" /></th>
								<th><s:text name="com.DailyHunt.scheduleAppointmentSummary.session2Date" /></th>
								

							</tr>
						</thead>
						<tbody>
							<c:forEach items="${dhstudentExtraDetailDTO}" var="studentDetails">
								<tr>
								<td style="text-align:center">  
								<!-- <a class="delete" href="javascript:void(0);" onclick='fnEditstudent("${studentDetails.id}", "${studentDetails.studentId})'>  -->
								 <a href="#" onclick='fnEditstudent("${studentDetails.id}", fnReplaceXMLEntities("${utils:replaceXMLEntities(studentDetails.studentId)}"))'> 
								...
								<td class="text-center">${utils:replaceXMLEntities(studentDetails.name)}</td>
								 
									<td class="text-center">
										${utils:replaceXMLEntities(studentDetails.studentemailId)}</td>
									<td class="text-center">
										${utils:replaceXMLEntities(studentDetails.studentcontactNumber)}</td>
									
									<td class="text-center">
										${utils:replaceXMLEntities(studentDetails.session1DateStr)}</td>
									<td class="text-center">
										${utils:replaceXMLEntities(studentDetails.session2DateStr)}</td>
...
<script type="text/javascript">
	...
	function fnGetOpenSessionHtml(data) {
		var html = '<a href=javascript:void(0); onclick="fnOpenSessionModel('
	...
	function fnGetEditActionsHtml(id,userId) {
		
		var html = '<a class=delete href=javascript:void(0); onclick="fnEditstudent('
				+ data.id
				+ ', '
				+ data.userId
				+ ')"> <i class=glyphicon glyphicon-edit operation-column></i>'
	...
	function fnGetDeleteActionsHtml(data) {
		var html = '';
		if (data.isDelete) {
			html = '<a class=delete href=javascript:void(0); onclick="fnDeletestudent('
					+ data.id
					+ ', '
					+ data.userId
					+ ')"> <i class=glyphicon glyphicon-edit operation-column></i>'
	...
	function fnEditstudent(id, studentId) {
		$('#stdId').val(studentId);
		 
		form.action = '';
		form.action = "${pageContext.request.contextPath}/myapp/editDHStudentConversionAction";
		form.submit();
	}
	...
	function fnGetType(data) {
		var html = '<p style="font-family: verdana;">' + data.studentTypeStr
				+ "</p>";
		return html;
	}
	function fnGetEmail(data) {
		var html = '<p style="font-family: verdana;">' + data.fatheremailId
				+ "</p>";
		return html;
	}
	function fnpersonalityCode(data) {
		var html = '<p style="font-family: verdana;">' + data.personalityCode
				+ "</p>";
		return html;
	}
	function fnGetSchool(data) {
		var html = '<p style="font-family: verdana;">' + data.schoolName
				+ "</p>";
		return html;
	}
	function fnGetLDID(data) {
		var html = '<p style="font-family: verdana;">' + data.loginId + "</p>";
		return html;
	}

	function fnGetName(data) {
		var html = '<p style="font-family: verdana;">' + data.name + "</p>";
		return html;
	}

	function fnGetPassword(data) {
		var html = '<p style="font-family: verdana;">' + data.passwordStr
				+ "</p>";
		return html;
	}
				...
				.ajax({
					...
					success : function(resp) {
						...
						if (data != "" && data != null) {
							...
							$("#ShowCompleteStudentdetails")
									.html(
											'<div class="modal-dialog modal-full"><div class="modal-content"><div class="modal-header"><h4 class="modal-title"><s:text name="com.edupath.student.summary.title"/><button type="button" class="btn btn-sm btn-danger" onclick="fnCloseSudentDetailsDialog();"style="float:right"><s:text name="X"/></button></h4></div><div class="modal-body"><table class="table table-striped table-hover table-bordered"><tr><td><s:text name="com.edupath.student.summary.name" /></td><td>'
													+ resp[0].name
													+ '</td></tr><tr><td><s:text name="com.edupath.student.summary.type" /></td><td>'
													+ resp[0].studentTypeStr
													+ '</td></tr><tr><td><s:text name="com.edupath.student.summary.email" /></td><td>'
													+ resp[0].fatheremailId
													+ '</td></tr><tr><td><s:text name="com.edupath.student.summary.school" /></td><td>'
													+ resp[0].schoolName
													+ '</td></tr><tr><td><s:text name="com.edupath.student.summary.session1" /></td><td>'
													+ resp[0].session1DateStr
													+ '</td></tr><tr><td><s:text name="com.edupath.student.summary.session2" /></td><td>'
													+ resp[0].session2DateStr
													+ '</td></tr><tr><td><s:text name="com.edupath.student.summary.session3" /></td><td>'
													+ resp[0].session3DateStr
													+ '</td></tr><tr><td><s:text name="com.edupath.student.summary.venue" /></td><td>'
													+ resp[0].venue
													+ '</td></tr><tr><td><s:text name="com.edupath.student.summary.dueamount" /></td><td>'
													+ dueamount

				...
				<table class="table table-striped table-hover table-bordered"
					...
					<thead>
						<tr>
							...
							<th>${pageScope.streamLabel}</th>
							<th>${pageScope.career1Label}</th>
							<th>${pageScope.career2Label}</th>
							<th>${pageScope.ugALabel}</th>
							<th>${pageScope.ugBLabel}</th>
							<th>${pageScope.electiveLabel}</th>
							<th>${pageScope.examLabel}</th>
						</tr>
					</thead>
					<tbody>
						<c:forEach items="${schoolReport}" var="schoolReportList">
							<tr>
								<td>${utils:replaceXMLEntities(schoolReportList.loginId)}</td>
								<td>${utils:replaceXMLEntities(schoolReportList.name)}</td>
								<td>
									${utils:replaceXMLEntities(schoolReportList.classLable)}</td>
								<td>${utils:replaceXMLEntities(schoolReportList.stream)}</td>
								<td>${utils:replaceXMLEntities(schoolReportList.career1)}</td>
								<td>${utils:replaceXMLEntities(schoolReportList.career2)}</td>
								<td>
									${utils:replaceXMLEntities(schoolReportList.ugDegreePlanA)}</td>
								<td>
									${utils:replaceXMLEntities(schoolReportList.ugDegreePlanB)}</td>
								<td>${utils:replaceXMLEntities(schoolReportList.elective)}
								</td>
								<td class="adj">
									${utils:replaceXMLEntities(schoolReportList.examRecommendations)}
								</td>
							</tr>
						</c:forEach>
					</tbody>
...
<script type="text/javascript">
	...
	function fnSearchReportBySchool() {
		...
		var urlVal = "${pageContext.request.contextPath}/myapp/searchDataCollegeReportAction?schoolId="+ schoolId + "&date=" + date;
		form.action = urlVal;
		form.submit();
	}
	...
	function fnExcelDownload() {
		...
		var urlVal = "${pageContext.request.contextPath}/myapp/excelDownloadCollegeReportAction?schoolId="+ schoolId + "&date=" + date;
		form.action = urlVal;
		form.submit();

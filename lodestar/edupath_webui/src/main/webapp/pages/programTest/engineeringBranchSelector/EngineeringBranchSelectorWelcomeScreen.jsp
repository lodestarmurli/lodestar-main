	...
	<div id="mainPage_1">
		...
		<div class="row">
			<div class="col-md-12 change_page_div_1">
				<c:choose>
					<c:when test="${!favSubCompleted}">
						<p class="change_p_h6"><strong class="change_p_h4"><b>Congratulations</b> </strong>you have chosen to make a scientific decision about the engineering branch you will take after 11th/12th. The test report will recommend the top engineering branches that are most suitable for you. </p>
						<br>
						...
						<c:choose>
							<c:when test="${!session.UserSessionObject.aptitudeCompleted}">
								<p class="change_p_h4"><b>Please make sure you fill in the Subject Priority after the Interest and Aptitude tests.</b>.</p>
							</c:when>
							<c:otherwise>
								<p class="change_p_h4" style="color: red"><b>Please make sure you fill in the Subject Priority after the Interest and Aptitude tests.</b>.</p>
							</c:otherwise>
						</c:choose>
						<br>
					...
					<c:otherwise>
					<br>
					<p class="change_p_h4">Based on your Vocational Personality and Aptitude Test results your<strong  class="change_p_h4"> <b>Engineering Branch Recommendation Report</b> </strong>has been generated by the Lodestar Career Guidance System.</p>
					
					<br>
					<c:choose>
						<c:when test="${!favSubCompleted}">
							<p class="change_p_h4" style='color:red'>Please complete Subject priority to download report</p> 
							<button class="btn btn_action" onclick= "" style="margin-right: 50px;" disabled>
						...
						<c:otherwise>
						<button class="btn btn_action" onclick="fnDownloadEngineeringSelectorReport();" style="margin-right: 50px;" >
					    		<span class="next-spn" style="padding-left:0px; padding-right:0px"><s:text name="com.edupath.streamselector.download.report.title"></s:text></span>
					   		</button>
						</c:otherwise>
					</c:choose>
...
<script>
	...
	function fnDownloadEngineeringSelectorReport()
	{
		var form = document.ReportForm;
		form.action = "${pageContext.request.contextPath}/myapp/EngineeringSelectorPDFReport";
		form.submit();
	}
</script>
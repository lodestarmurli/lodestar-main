...
<div class="feedback-form-main">
	<div class="row filter-margin-top">
		<div class="col-md-12 col-xs-12 college-search-header">    <!-- 23/03/18 added col-xs-12 -->
			<s:text name="com.edupath.feedbackform.parentfeedbackformsessionone.title"/> 
		</div>
	</div>
			...
			<s:iterator value="feedbackQuestions" var="fbquestion" status="counter">
				<div class="feedback-question-div form-group">
					...
					<div>
						...
						<s:if test="%{#fbquestion.questionNo eq 2}">
							<div class="feedback-star-rating">
									...
									<select class="example-fontawesome-o" name="${fbquestion.questionNo}_${subQuestion}">
							                 <option value="1" data-subtype="${fbquestion.questionNo}" data-questionno="${fbquestion.questionNo}">1</option>
							                 <option value="2" data-subtype="${fbquestion.questionNo}" data-questionno="${fbquestion.questionNo}">2</option>
							                 <option value="3" data-subtype="${fbquestion.questionNo}" data-questionno="${fbquestion.questionNo}">3</option>
							                 <option value="4" data-subtype="${fbquestion.questionNo}" data-questionno="${fbquestion.questionNo}">4</option>
							                 <option value="5" data-subtype="${fbquestion.questionNo}" data-questionno="${fbquestion.questionNo}">5</option>
						...
						<s:if test="%{#fbquestion.questionNo eq 4}">
							<div class="feedback-star-rating">
								
									<div class="feedback-star-rating-row">
									<select class="example-fontawesome-o" name="${fbquestion.questionNo}">
							                 <option value="1" data-subtype="${fbquestion.questionNo}" data-questionno="${fbquestion.questionNo}">1</option>
							                 <option value="2" data-subtype="${fbquestion.questionNo}" data-questionno="${fbquestion.questionNo}">2</option>
							                 <option value="3" data-subtype="${fbquestion.questionNo}" data-questionno="${fbquestion.questionNo}">3</option>
							                 <option value="4" data-subtype="${fbquestion.questionNo}" data-questionno="${fbquestion.questionNo}">4</option>
...
<s:form action="/myapp/Session1" method="POST" name="sessionFeedbackFormAction">
...
<script type="text/javascript">
	...
	$('#feedback_submit_button').click(function()
	{
		...
		if(dataJSON!=null && dataJSON.FEED_BACK.length==4)
		{
	...
	function fnFinalizeStudentFeedBack(dataParam)
	{
		...
		var actionUrl = "${pageContext.request.contextPath}/myapp/SaveSessionOneFeedBack";
		$.ajax({
			url : actionUrl,
			type : "POST",
